...
const BACKEND_BASE_URL = process.env.BACKEND_BASE_URL || 'http://localhost:5001/api/v1'
const BACKEND_TOKEN =
...
export async function GET(request: Request) {
  try {
    ...
    const fetchBooks = async (categoryId?: string) => {
      ...
      const data = (await res.json().catch(() => null)) as BackendBooksPayload | null
      return { res, data }
    }
    ...
    if (selectedCategories.length > 1) {
      ...
      if (failed) {
        return NextResponse.json(
          { message: failed.data?.message || 'Failed to fetch books from backend' },
      ...
      const deduped = Array.from(new Map(merged.map(item => [item._id || Math.random(), item])).values())
    ...
    const { res: response, data: payload } = await fetchBooks(selectedCategories[0])
