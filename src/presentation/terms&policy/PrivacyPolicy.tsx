  ...
  return (
    <div className="w-full bg-white font-sans">
      <div className="py-10 text-center border-b border-gray-100 bg-[#FAFAFA]">
        <h1 className="text-4xl font-serif text-gray-900 mb-2">{t('policy.privacyTitle')}</h1>
        <p className="text-gray-600 text-lg">{t('policy.privacySub')}</p>
      </div>
            ...
            {sections.map((id) => (
              <div key={id}>
                <h3 className="text-xl font-bold text-gray-800 mb-3 font-serif">
                  {id}. {t(`policy.privacy.s${id}.title`)}
                </h3>
                <p className="text-gray-500 leading-relaxed text-base">
                  {t(`policy.privacy.s${id}.content`)}
                </p>
              </div>
            ))}
          </div>
